...
from .tasks import create_scsgl_task
...
# Initialise environment variables
    ...
    requests_cache.install_cache('scsgl_cache')
    ...
    class RunAlgorithm(Resource):
        ...
        def post(self):
            ...
            pos_density = args.get('pos_density')
            neg_density = args.get('neg_density')
            assoc = args.get('assoc')
            if not pos_density:
                pos_density = 0.45
            if not neg_density:
                neg_density = 0.45
            ...
            task = create_scsgl_task.delay(args["zenodo_id"], pos_density, neg_density, assoc)
    ...
    class AlgorithmInfo(Resource):
        ...
        def get(self):
            ...
            info = {
                    "name": 'scsgl',
                    "description": 'The current implementation of scSGL takes an expression matrix and a list of genes as input. It produces a ranked list of edges as output, which we used directly in our analysis.',
                    "edge_weight_type": '',
